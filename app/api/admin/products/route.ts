...
export const GET = authMiddleware(async (req: NextRequest) => {
  try {
    ...
    const products = await Product.find(query).sort({ order: 1, createdAt: -1 }).skip(skip).limit(limit)
...
export const POST = authMiddleware(async (req: NextRequest) => {
  try {
    ...
    if ((error as any)?.name === "ValidationError") {
      const errs = Object.values((error as any).errors || {}).map((e: any) => e.message)
      return NextResponse.json({ error: "Product validation failed", details: errs }, { status: 400 })
    }
    return NextResponse.json({ error: "Internal server error" }, { status: 500 })
  }
})
